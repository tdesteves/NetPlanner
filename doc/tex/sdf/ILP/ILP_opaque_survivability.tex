...
The objective function of following the ILP is a minimization of the sum of two equations: the cost of the links \ref{Capex_Link} and cost of the nodes \ref{Capex_Node} where in this case we only have in consideration the electric cost \ref{Capex_Node_EXC} because for any value of $n$ the $N_{oxc,n}$ will always be 0 so the optical cost is zero.
...
\newpage
\begin{equation}
P_{exc,-1,n} = \sum_{k=1}^{N} W_{nk}
\label{EXC_pexc1_opaque}
\end{equation}

\begin{itemize}
\item{$P_{exc,-1,n}$	$\rightarrow$	Number of ports of the electrical switch long-reach}
\item{$k$	      		$\rightarrow$	Source node}
\item{$W_{nk}$			$\rightarrow$	Number of optical channels between $n$ and $k$}
\end{itemize}


\begin{equation}
P_{exc,c,n} = \sum_{c=0}^{4} D_{od,c}
\label{EXC_pexc2_opaque}
\end{equation}

\begin{itemize}
\item{$P_{exc,c,n}$	$\rightarrow$	Number of ports of the electrical switch sort-reach}
\item{$D_{od,c}$	$\rightarrow$	client demands with bit rate $c$ between nodes $o$ and $d$}
\end{itemize}
