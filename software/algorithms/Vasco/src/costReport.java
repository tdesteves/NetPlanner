...
{	
	...
	*/
	...
	public String executeReport(NetPlan netPlan, Map<String, String> reportParameters, Map<String, String> net2planParameters)
	{
		...
		double quantityODU0low = 60;
		double quantityODU1low = 50;
		double quantityODU2low = 16;
		double quantityODU3low = 6;
		double quantityODU4low = 4;
		double quantityODU0medium = 600;
		double quantityODU1medium = 500;
		double quantityODU2medium = 160;
		double quantityODU3medium = 60;
		double quantityODU4medium = 40;
		double quantityODU0high = 1200;
		double quantityODU1high = 1000;
		double quantityODU2high = 320;
		double quantityODU3high = 120;
		double quantityODU4high = 80;
		
		double CostPEXC = N * unitPricePEXC;
		double CostPTribODU0low = quantityODU0low * unitPriceODU0 * 1.25;
		double CostPTribODU1low = quantityODU1low * unitPriceODU1 * 2.5;
		double CostPTribODU2low = quantityODU2low * unitPriceODU2 * 10;
		double CostPTribODU3low = quantityODU3low * unitPriceODU3 * 40;
		double CostPTribODU4low = quantityODU4low * unitPriceODU4 * 100;
		double CostPTribODU0medium = quantityODU0medium * unitPriceODU0 * 1.25;
		double CostPTribODU1medium = quantityODU1medium * unitPriceODU1 * 2.5;
		double CostPTribODU2medium = quantityODU2medium * unitPriceODU2 * 10;
		double CostPTribODU3medium = quantityODU3medium * unitPriceODU3 * 40;
		double CostPTribODU4medium = quantityODU4medium * unitPriceODU4 * 100;
		double CostPTribODU0high = quantityODU0high * unitPriceODU0 * 1.25;
		double CostPTribODU1high = quantityODU1high * unitPriceODU1 * 2.5;
		double CostPTribODU2high = quantityODU2high * unitPriceODU2 * 10;
		double CostPTribODU3high = quantityODU3high * unitPriceODU3 * 40;
		double CostPTribODU4high = quantityODU4high * unitPriceODU4 * 100;
		...
		if (N > 0)
		{
			...
			for(long nodeId : nodeIds)
			{
				...
				for(int j=0;j<nodes.size();j++)
				{
					...
					{
						if(type.equals(transparent)) portsTribIn[netPlan.getNodeFromId(nodeId).getIndex()] += (int) Math.round(trafficR[j]/80);
					...
					{
						if(type.equals(transparent)) portsTribOut[netPlan.getNodeFromId(nodeId).getIndex()] += (int) Math.round(trafficR[j]/80);
						if(type.equals(opaque))
						{
							int a=0;
				...
				if(type.equals(opaque))
				{
					...
					{
						portsLink[b] = (int)Math.round(portsLink[b]);
					}
					portsTribIn[netPlan.getNodeFromId(nodeId).getIndex()] = DoubleUtils.sum(portsLink);	
					for(int b=0;b<portsLink1.length;b++)
					{
						portsLink1[b] = (int)Math.round(portsLink1[b]);
					}
					portsTribOut[netPlan.getNodeFromId(nodeId).getIndex()] = DoubleUtils.sum(portsLink1);   			  			
				}
				portsTribIn[netPlan.getNodeFromId(nodeId).getIndex()] = (int) Math.round(portsTribIn[netPlan.getNodeFromId(nodeId).getIndex()]);
				portsTribOut[netPlan.getNodeFromId(nodeId).getIndex()] = (int) Math.round(portsTribOut[netPlan.getNodeFromId(nodeId).getIndex()]);
		...
		double costPLine = nOpticalChannels * 1000 * Tau;
		...
		if(type.equals(opaque))
		{
			...
			Cexc = CostPEXC + costPLine + CostPTribODU0low + CostPTribODU1low + CostPTribODU2low + CostPTribODU3low + CostPTribODU4low;
		}
		...
		costInformationTable.append(String.format("<tr> <td><b>ODU0 Ports</b></td> <td>%s</td> <td>%s €/Gb/s</td> <td>%s €</td>", (int) quantityODU0low, (int) unitPriceODU0, NumberFormat.getIntegerInstance().format(CostPTribODU0low)));
		costInformationTable.append(String.format("<tr> <td><b>ODU1 Ports</b></td> <td>%s</td> <td>%s €/Gb/s</td> <td>%s €</td>", (int) quantityODU1low, (int)unitPriceODU1, NumberFormat.getIntegerInstance().format(CostPTribODU1low)));
		costInformationTable.append(String.format("<tr> <td><b>ODU2 Ports</b></td> <td>%s</td> <td>%s €/Gb/s</td> <td>%s €</td>", (int) quantityODU2low, (int) unitPriceODU2, NumberFormat.getIntegerInstance().format(CostPTribODU2low)));
		costInformationTable.append(String.format("<tr> <td><b>ODU3 Ports</b></td> <td>%s</td> <td>%s €/Gb/s</td> <td>%s €</td>", (int) quantityODU3low, Double.toString(unitPriceODU3), NumberFormat.getIntegerInstance().format(CostPTribODU3low)));
		costInformationTable.append(String.format("<tr> <td><b>ODU4 Ports</b></td> <td>%s</td> <td>%s €/Gb/s</td> <td>%s €</td>", (int) quantityODU4low, (int) unitPriceODU4, NumberFormat.getIntegerInstance().format(CostPTribODU4low)));
